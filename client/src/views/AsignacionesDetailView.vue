...
const loadAsignacion = async () => {
    ...
    try {
        ...
        // Cargar componentes si existen (solo si es asignación activa o queremos ver historial de componentes - para ahora solo activos/actuales)
        // El endpoint getComponentes trae los activos actualmente vinculados
        try {
            componentes.value = await AsignacionesService.getComponentes(id)
        } catch (e) {
            // Si falla components (ej. 404 porque no tiene), no bloquemos la vista
...
const finalizarAsignacion = () => {
    confirm.require({
        message: '¿Está seguro de finalizar esta asignación? El equipo y la IP quedarán disponibles.',
        ...
        accept: async () => {
            try {
                ...
                loadAsignacion() // Recargar para ver estado actualizado
            } catch (error) {
...
<template>
    <div class="animate-fade-in-up">
        ...
        <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-3 bg-white dark:bg-dark-card rounded-lg shadow p-6 border border-gray-200 dark:border-dark-border" v-if="componentes.length > 0">
                 <h3 class="text-lg font-bold mb-4 flex items-center gap-2">
                    <i class="pi pi-box text-orange-500"></i> Componentes Adicionales
                </h3>
                <DataTable :value="componentes" size="small" class="text-sm">
