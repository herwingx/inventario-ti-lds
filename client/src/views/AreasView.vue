  ...
  <div class="animate-fade-in-up">
    ...
    <div class="bg-white dark:bg-dark-card rounded-lg shadow-xl p-6 border border-gray-200 dark:border-dark-border transition-colors duration-300">
      ...
      <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <Button label="Nueva Área" icon="pi pi-plus" class="!bg-primary !border-none hover:!bg-primary-hover !font-bold !px-6 !py-2.5 !rounded-lg !text-white !text-sm shadow-lg shadow-emerald-900/20 w-full md:w-auto" @click="openNew" />
      </div>
        ...
        <Column header="Acciones" style="width: 15%; text-align: right">
            <template #body="{ data }">
                ...
                <div v-else class="flex gap-1 justify-start">
                    <!-- View Button -->
                    <button class="w-7 h-7 rounded bg-gray-100 dark:bg-dark-bg hover:bg-blue-50 dark:hover:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center transition-all border border-gray-200 dark:border-transparent hover:border-blue-500" @click="viewArea(data)" title="Ver detalles">
                        <i class="pi pi-eye text-xs"></i>
                    </button>
                    <!-- Edit Button -->
                    <button class="w-7 h-7 rounded bg-gray-100 dark:bg-dark-bg hover:bg-gray-200 dark:hover:bg-dark-border text-primary flex items-center justify-center transition-all border border-gray-200 dark:border-transparent" @click="editArea(data)" title="Editar">
                        <i class="pi pi-pencil text-xs"></i>
                    </button>
                    <!-- Delete Button -->
                    <button class="w-7 h-7 rounded bg-gray-100 dark:bg-dark-bg hover:bg-red-50 dark:hover:bg-red-900/30 text-red-500 flex items-center justify-center transition-all border border-gray-200 dark:border-transparent hover:border-red-500" @click="confirmDeleteArea(data)" title="Eliminar">
