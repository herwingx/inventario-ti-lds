...
const columns = [
  ...
  { field: 'nombre', header: 'Nombre del Área', sortable: true, width: '35%' },
  { field: 'nombre_empresa', header: 'Empresa', sortable: true, width: '30%' },
  { field: 'status_nombre', header: 'Estado', sortable: true, width: '12%' },
...
const filteredAreas = computed(() => {
  ...
  if (globalFilter.value) {
    ...
    result = result.filter(a => 
      ...
      a.nombre_empresa?.toLowerCase().includes(search)
    )
  }

  if (statusFilter.value) {
    result = result.filter(a => a.status_nombre?.toUpperCase() === statusFilter.value)
  }
    ...
    <div class="bg-white dark:bg-dark-card rounded-lg shadow-xl p-6 border border-gray-200 dark:border-dark-border transition-colors duration-300">
      ...
      <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto flex-1">
          <div class="relative w-full sm:w-72">
            ...
            <InputText v-model="globalFilter" placeholder="Buscar área, empresa..." class="w-full !pl-10 !bg-gray-50 dark:!bg-dark-bg !border-gray-300 dark:!border-dark-border !text-gray-900 dark:!text-white !py-2.5 !rounded-lg focus:!border-primary" />
          </div>
      ...
      >
        ...
        <template #nombre="{ data }">
          <div class="text-gray-900 dark:text-white font-bold text-base">{{ data.nombre }}</div>
        </template>
        ...
        <template #nombre_empresa="{ data }">
          <span class="text-gray-800 dark:text-gray-200 text-sm font-bold bg-gray-100 dark:bg-gray-800/50 px-3 py-1.5 rounded-md border border-gray-200 dark:border-gray-700">{{ data.nombre_empresa }}</span>
        </template>

        <template #skeleton-nombre_empresa>
          <div class="skeleton h-6 w-24 rounded-md"></div>
        </template>
